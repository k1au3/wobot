...
const { createUserSchema, createOrderSchema, updateUserSchema, validateLogin } = require('../middleware/validators/userValidator.middleware');


router.get('/', auth(), awaitHandlerFactory(userController.getAllUsers)); // localhost:3000/api/v1/users
router.get('/order', awaitHandlerFactory(userController.getAllOrders)); // localhost:3000/api/v1/users
router.get('/id/:id', auth(), awaitHandlerFactory(userController.getUserById)); // localhost:3000/api/v1/users/id/1
router.get('/order/:id', awaitHandlerFactory(userController.getOrderById)); // localhost:3000/api/v1/users/id/1
...
router.post('/order', createOrderSchema, awaitHandlerFactory(userController.createOrder)); // localhost:3000/api/v1/users/order
...
router.delete('/order/:id', awaitHandlerFactory(userController.deleteOrder)); // localhost:3000/api/v1/users/id/1
router.delete('/order/all', awaitHandlerFactory(userController.deleteAllOrders)); // localhost:3000/api/v1/users/id/1
